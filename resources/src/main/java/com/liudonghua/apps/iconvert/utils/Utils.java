...
public class Utils {
	...
	public static ResourceBundle loadResourceBundle(String rootPath, String resourceBaseName, Locale locale) {
	...
	public static void convertEncoding(String sourceFilePath, String targetFilePath, String fromEncoding,
			String toEncoding) {
    	// use commons-exec instead of Runtime.getRuntime().exec(cmd) or ProcessBuilder
    	// http://stackoverflow.com/questions/3468987/executing-another-application-from-java
    	String cmdLineStr = String.format("\"%s\" -f \"%s\" -t \"%s\" \"%s\" > \"%s\"", getIconvExecutablePath(), fromEncoding, toEncoding, sourceFilePath, targetFilePath);
	...
	private static String getIconvExecutablePath() {
		if(iconvExecutablePath == null) {
			...
			try {
				...
				if(isWindows()) {
					String rootPath = new File(Utils.class.getProtectionDomain().getCodeSource().getLocation().getPath()).getParent();
					iconvExecutablePath = new File(rootPath , "resources/bin/iconv.exe").getAbsolutePath();
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public static String getCustomConvertTargetFilePath(String rootPath, String outputRootPath, String filePath) {
