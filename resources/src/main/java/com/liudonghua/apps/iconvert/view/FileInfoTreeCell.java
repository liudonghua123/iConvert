...
public class FileInfoTreeCell extends TreeCell<FileInfoWrapper> {
	...
	protected void updateItem(FileInfoWrapper item, boolean empty) {
		...
		if (empty || item == null) {
			...
			treeViewCellController.getTreeViewCellCheckBox().setOnAction((event) -> updateState(event));
    ...
    private void updateState(ActionEvent event) {
        ...
        if (treeItem.getValue().getTreeViewCellController().getTreeViewCellCheckBox().indeterminateProperty().get()) return;
    ...
    private void updateUpwards(TreeItem<FileInfoWrapper> treeItem) {
        ...
        if(parent != null) {
	        CheckBox parentCheckBox = parent.getValue().getTreeViewCellController().getTreeViewCellCheckBox();
	        int selectCount = 0;
	        int indeterminateCount = 0;
	        for (TreeItem<FileInfoWrapper> child : parent.getChildren()) {
	        	CheckBox cbti = child.getValue().getTreeViewCellController().getTreeViewCellCheckBox();
    ...
    private void updateDownwards(TreeItem<FileInfoWrapper> treeItem) {
        if (!treeItem.isLeaf()) {
            for (TreeItem<FileInfoWrapper> child : treeItem.getChildren()) {
	        	CheckBox cbti = child.getValue().getTreeViewCellController().getTreeViewCellCheckBox();
                cbti.setSelected(treeItem.getValue().getTreeViewCellController().getTreeViewCellCheckBox().isSelected());
