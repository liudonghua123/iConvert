package com.liudonghua.apps.lab;
...
//                item -> new Observable[] {item.valueProperty() }));
        table.setItems(FXCollections.observableArrayList(new Callback<Item, Observable[]>() {
			@Override
			public Observable[] call(Item item) {
				return new Observable[] {item.valueProperty() };
			}
...
//            table.getItems());
        IntegerBinding total = Bindings.createIntegerBinding(new Callable<Integer>() {
			...
			public Integer call() throws Exception {
				return table.getItems().stream().collect(Collectors.summingInt(Item::getValue));
			}
